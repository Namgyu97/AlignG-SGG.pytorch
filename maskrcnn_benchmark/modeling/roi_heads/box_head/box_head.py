    ...
    def forward(self, features, proposals, targets=None):
        ...
        if self.cfg.MODEL.RELATION_ON:
            if self.cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
                ...
                # VG targets carry "attributes"; GQA targets do not. Detect from the data
                # so VG-150 keeps its original code path while GQA-200 falls back gracefully.
                has_attr = all(t.has_field("attributes") for t in targets)
                fields = ["labels", "attributes"] if has_attr else ["labels"]
                proposals = [target.copy_with_fields(fields) for target in targets]
